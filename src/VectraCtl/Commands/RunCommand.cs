...
internal static class RunCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Resolve the tag: prefer the saved tag, fall back to latest release
        var tag = ds.Tag;
        ...
        else if (!await docker.IsContainerRunningAsync(containerName, cancellationToken))
        {
            ...
            {
                logger.WriteError(string.IsNullOrWhiteSpace(startResult.Error)
        ...
        // Persist resolved settings so subsequent commands can use them
